...
from src.python_telegrambot_petlin.tgbot_petlin.tgbot_petlin.Settings.config import settings as SETTINGS
#from src.python_telegrambot_petlin.tgbot_petlin.tgbot_admin.models import BotStatistics
...
commands = [
    ...
    BotCommand("del_events", "Удалить событие"),
]
...
async def add_note(update: Update, context: CallbackContext) -> None:
    ...
    cursor.execute(f'insert into notes (uid, uname, date_note, note, time_note) values ({user_id}, \'{username}\', CURRENT_DATE, \'{user_text}\', \'{formatted_time}\');')
    conn.commit()
    cursor.close()
    conn.close()
...
async def add_event(update: Update, context: CallbackContext) -> None:
    ...
    cursor.execute(f'insert into events (uid, uname, event_date, event_time, details) values ({user_id}, '
...
async def del_event(update: Update, context: CallbackContext) -> None:
    ...
    if user_text.isdigit():
        ...
        try:
            ...
            print(psycopg2.OperationalError)
        cursor.execute(f'SELECT * FROM events WHERE uid={user_id} and id={user_text};', (1,))

        rows = cursor.fetchall()
        if rows:
            cursor.execute(f'DELETE FROM events WHERE uid={user_id} and id={user_text};')
            conn.commit()
            result += (f'*Заметка №{str(rows[0][0])}:* _\"{str(rows[0][4])}\"_* удалена*')
        else:
            result += (f'Сочетание {user_text} и {user_id} для пользователя {username} не найдено')
    ...
    if rows:
        if table == 'notes':
            for row in sorted(rows):
                ...
                all_notes_str += '*' + row_id + '*' + '\t' + row_date + '\t' + row_time + '\t' + '_' + row_text + '_' + '\n'
